...
export class NotificationsController {
    ...
    async markAllRead(@Req() req: any) {
        const data = await this.notificationsService.markAllRead(req.user.userId);
        return { success: true, data };
    }
    @Delete('me/clear-all')
    @UseGuards(JwtAuthGuard)
    async clearAllMine(@Req() req: any) {
        const data = await this.notificationsService.clearAll(req.user.userId);
        return { success: true, data };
    }
    ...
    async removeMine(@Req() req: any, @Param('id') id: string) {
        const data = await this.notificationsService.removeOne(req.user.userId, id);
        return { success: true, data };
    }
    @Patch('me/:id/read')
    @UseGuards(JwtAuthGuard)
    async markRead(@Req() req: any, @Param('id') id: string) {
        const data = await this.notificationsService.markRead(req.user.userId, id);
        return { success: true, data };
    }
}