...
export class GithubOAuthGuard extends AuthGuard('github') {
    private isCallback(req: Request) {
        return req.path.endsWith('/github/callback');
    }
    ...
    async canActivate(context: ExecutionContext) {
        ...
        if (!this.isCallback(req)) {
            ...
            // cookie ctx fallback cho callback
    ...
    getAuthenticateOptions(context: ExecutionContext) {
        ...
        if (!actor || !app) {
            throw new BadRequestException('Missing actor/app');
        }

        return {
            session: false,
            state: encodeState({ actor, app, from, deviceId }),
        };
    }
}