...
import AdminVoucherModal from "@/components/promotion/AdminVoucherModal";
...
export default function PlatformVouchers() {
    ...
    const promoMap = useMemo(() => {
        ...
        (promotions ?? []).forEach((p: any) => m.set(String(p._id), p));
        return m;
    }, [promotions]);
    ...
    const filtered = useMemo(() => {
        ...
        let rows = vouchers ?? [];
        if (tab === "active") rows = rows.filter((v: any) => !!v.is_active);
        if (tab === "inactive") rows = rows.filter((v: any) => !v.is_active);
        if (!key) return rows;
        return rows.filter((v: any) => String(v.code ?? "").toLowerCase().includes(key));
    }, [vouchers, tab, q]);
    ...
    const columns = [
        {
            ...
            render: (v: string) => (
                <span style={{ fontFamily: "monospace", fontWeight: 600 }}>
                    <Tag color="cyan" className="text-lg">{v}</Tag>
                </span>
            ),
        },
        {
            ...
            render: (pid: any) => {
                ...
                return p ? (
                    <div>
                        <Tag color="blue" className="text-[16px] font-semibold">{p.name}</Tag>
    ...
    return (
        <div style={{ padding: 12 }}>
            ...
            >
                <div>
                    <h2 style={{ margin: 0 }}>Platform Vouchers</h2>
                    <div style={{ color: "#777" }}>
                        Super Admin quản lý voucher platform. Chỉ chọn promotion có activation_type = voucher
                    </div>
                </div>
