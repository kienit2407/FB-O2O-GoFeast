...
class _OrderChatPageState extends ConsumerState<OrderChatPage> {
  ...
  Future<void> _send() async {
    ...
    _textCtrl.clear();
    await ref
        .read(driverOrderChatControllerProvider(widget.orderId).notifier)
        .send(text);
    _scrollToEnd();
  }
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(driverOrderChatControllerProvider(widget.orderId));
    ref.listen(driverOrderChatControllerProvider(widget.orderId), (
      previous,
      next,
    ) {
            ...
            child: state.isLoading
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _ChatBubble(
                        message: message,
                        isMine: message.senderRole == 'driver',
                        onRetry:
                            message.deliveryStatus ==
                                OrderChatDeliveryStatus.failed
                            ? () => ref
                                  .read(
                                    driverOrderChatControllerProvider(
                                      widget.orderId,
                                    ).notifier,
                                  )
                                  .retry(message)
